...
class RInteractionController{
    ...
    private final RUiAutomatorBridge mUiAutomatorBridge;
    ...
    public RInteractionController(RUiAutomatorBridge bridge) {
     ...
     */
    public boolean hoverAndSync(final int x, final int y, long timeout) {

        String logString = String.format("clickAndSync(%d, %d)", x, y);
        Log.d(LOG_TAG, logString);

        return runAndWaitForEvents(hoverRunnable(x, y), new WaitForAnyEventPredicate(
    ...
    public boolean doubleclickAndSync(final int x, final int y, long timeout) {

        String logString = String.format("clickAndSync(%d, %d)", x, y);
        Log.d(LOG_TAG, logString);

        return runAndWaitForEvents(doubleclickRunnable(x, y), new WaitForAnyEventPredicate(
                AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED |
                        AccessibilityEvent.TYPE_VIEW_SELECTED), timeout) != null;
    }

    public boolean rightclickAndSync(final int x, final int y, long timeout) {

        String logString = String.format("clickAndSync(%d, %d)", x, y);
        Log.d(LOG_TAG, logString);

        return runAndWaitForEvents(rightclickRunnable(x, y), new WaitForAnyEventPredicate(
                AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED |
                        AccessibilityEvent.TYPE_VIEW_SELECTED), timeout) != null;
    }

    public boolean leftclickAndSync(final int x, final int y, long timeout) {

        String logString = String.format("clickAndSync(%d, %d)", x, y);
        Log.d(LOG_TAG, logString);

        return runAndWaitForEvents(leftclickRunnable(x, y), new WaitForAnyEventPredicate(
                AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED |
                        AccessibilityEvent.TYPE_VIEW_SELECTED), timeout) != null;
    }
     ...
     * @return Runnable
     */
    private Runnable hoverRunnable(final int x, final int y) {
        return new Runnable() {
            @Override
            public void run() {
                hoveron(x, y);
    ...
    private Runnable doubleclickRunnable(final int x, final int y) {
        return new Runnable() {
            @Override
            public void run() {
                if(touchDown(x, y)) {
    ...
    private Runnable rightclickRunnable(final int x, final int y) {
        return new Runnable() {
            @Override
            public void run() {
                if(righttouchDown(x, y)) {
                    SystemClock.sleep(REGULAR_CLICK_LENGTH);
                    righttouchUp(x, y);
                }
            }
    ...
    private Runnable leftclickRunnable(final int x, final int y) {
        return new Runnable() {
            @Override
            public void run() {
                if(lefttouchDown(x, y)) {
                    SystemClock.sleep(REGULAR_CLICK_LENGTH);
                    lefttouchUp(x, y);
                }
            }
    ...
    private boolean hoveron(int x, int y) {
        ...
        MotionEvent event = MotionEvent.obtain(
                mDownTime, mDownTime, MotionEvent.ACTION_HOVER_ENTER, x, y, 0);
    ...
    private boolean righttouchDown(int x, int y) {
        if (DEBUG) {
            Log.d(LOG_TAG, "touchDown (" + x + ", " + y + ")");
        }
        mDownTime = SystemClock.uptimeMillis();
        /*MotionEvent event = MotionEvent.obtain(
                mDownTime, mDownTime, MotionEvent.BUTTON_SECONDARY, x, y, 0);*/
        PointerCoords pointerCoordstemp = new PointerCoords();
        pointerCoordstemp.x=x;
        pointerCoordstemp.y=y;
        PointerCoords[] pointerCoordsgroup=new PointerCoords[1];
        pointerCoordsgroup[0]=pointerCoordstemp;

        PointerProperties pointerPropertiestemp = new PointerProperties();
        pointerPropertiestemp.id=0;
        pointerPropertiestemp.toolType= MotionEvent.TOOL_TYPE_MOUSE;
        PointerProperties[] pointerPropertiesgroup = new PointerProperties[1];
        pointerPropertiesgroup[0] = pointerPropertiestemp;

        MotionEvent event = MotionEvent.obtain(mDownTime, mDownTime,MotionEvent.ACTION_DOWN, 1,
                pointerPropertiesgroup, pointerCoordsgroup, 0, MotionEvent.BUTTON_SECONDARY,1, 1, 0, 0x0,0x2002,0x0);

        Log.d(LOG_TAG, event.toString());
        return injectEventSync(event);
    }

    private boolean righttouchUp(int x, int y) {
        if (DEBUG) {
            Log.d(LOG_TAG, "touchDown (" + x + ", " + y + ")");
        }
        mDownTime = SystemClock.uptimeMillis();
        ...
        MotionEvent event = MotionEvent.obtain(mDownTime, mDownTime,MotionEvent.ACTION_UP, 1,
                pointerPropertiesgroup, pointerCoordsgroup, 0, MotionEvent.BUTTON_SECONDARY,1, 1, 0, 0x0,0x2002,0x0);
        Log.d(LOG_TAG, event.toString());
        return injectEventSync(event);
    }

    private boolean lefttouchDown(int x, int y) {
        if (DEBUG) {
            Log.d(LOG_TAG, "touchDown (" + x + ", " + y + ")");
        }
        mDownTime = SystemClock.uptimeMillis();
        ...
        MotionEvent event = MotionEvent.obtain(mDownTime, mDownTime,MotionEvent.ACTION_DOWN, 1,
                pointerPropertiesgroup, pointerCoordsgroup, 0, MotionEvent.BUTTON_PRIMARY,1, 1, 0, 0x0,0x2002,0x0);

        Log.d(LOG_TAG, event.toString());
        return injectEventSync(event);
    }

    private boolean lefttouchUp(int x, int y) {
        if (DEBUG) {
            Log.d(LOG_TAG, "touchDown (" + x + ", " + y + ")");
        }
        mDownTime = SystemClock.uptimeMillis();
